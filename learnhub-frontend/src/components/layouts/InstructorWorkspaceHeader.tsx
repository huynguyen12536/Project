...
import { getInstructorStudioLabel } from './instructorStudioNavigation';

interface InstructorWorkspaceHeaderProps {
  title: string;
}

export const InstructorWorkspaceHeader: React.FC<InstructorWorkspaceHeaderProps> = ({ title }) => {
  ...
  const displayName = useMemo(() => {
    ...
    return fullName || user?.email || 'Tai khoan';
  ...
  const breadcrumbLabel = title || getInstructorStudioLabel(location.pathname);
    ...
    <header className="sticky top-0 z-40 border-b border-[#E5E7EB] bg-white/95 backdrop-blur">
      <div className="flex h-[72px] items-center gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden min-w-0 flex-1 lg:block">
          <div className="mx-auto max-w-xl">
            <div className="relative">
              ...
              <input
                type="search"
                placeholder="Tim khoa hoc, bai giang, hoc vien trong studio..."
        ...
        <div className="ml-auto flex items-center gap-3">
          <button
            onClick={() => navigate('/')}
            className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#E5E7EB] bg-white px-3.5 text-sm font-medium text-[#374151] transition hover:bg-[#F9FAFB]"
          >
            <Home className="h-4 w-4" />
            <span className="hidden sm:inline">Xem trang chu</span>
          </button>
            ...
            <AnimatePresence>
              {accountOpen ? (
                ...
                >
                  ...
                  >
                    <Sparkles className="h-4 w-4 text-lh-blue" />
                    Hoc tap
                  </button>
                  <button
                    onClick={() => {
...
export default InstructorWorkspaceHeader;
